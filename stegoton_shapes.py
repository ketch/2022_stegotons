...
def moving_wall_bc(state,dim,t,qbc,auxbc,num_ghost):
    ...
    shape = state.problem_data['shape'] 
    ...
    if dim.on_lower_boundary:
        ...
        if abs(t0)<=1.: 
            if shape == "sech2":
                wid = 1.05*np.sqrt(a1/1.32)
                vwall = -a1/(np.cosh(wid*(t-3.)))**2#/rhoB
                #vwall = -fac*1.32/np.cosh(np.sqrt(fac)*1.05*(t-3.))**2/rhoB
            elif shape == "cos2":
                wid = 0.41*np.sqrt(a1/0.087)
                #wid = 0.7*np.sqrt(a1/0.45)
                if np.abs(wid*(t-3)) < np.pi/2:
                    vwall = -a1*(np.cos(wid*(t-3.)))**2#/rhoB
                else:
                    vwall = 0.
                #vwall = -fac*3./np.cosh(np.sqrt(fac)*0.3*(t-3.))**30/rhoB
            elif shape == "cosp":
                wid = 0.83*np.sqrt(a1/0.45)
                #cos = np.maxiumum(np.cos(wid*(t-3)),0)
                if np.abs(wid*(t-3)) < np.pi/2:
                    vwall = -a1*(np.cos(wid*(t-3.)))**1.6
                else:
                    vwall = 0.
            elif shape == "cos1":
                if np.abs((t-3)) < np.pi/2:
                    vwall = -a1*(np.cos((t-3.)))
                else:
                    vwall = 0.

            else:
                vwall = -a1*(1.+np.cos(t0*np.pi))
        else: 
            vwall=0.
        for ibc in range(num_ghost-1):
            qbc[1,num_ghost-ibc-1] = 2*vwall*state.aux[1,ibc] - qbc[1,num_ghost+ibc]
            #sigg = 2./(np.cosh(np.sqrt(2.)*(t-3.)))**2
            #qbc[0,num_ghost-ibc-1] = np.log(sigg+1)/state.aux[1,num_ghost-ibc-1]



def setup(use_petsc=0,kernel_language='Fortran',solver_type='classic',outdir='./_output',ic=1,
            tfinal=1000.,num_output_times=2000, cells_per_layer=40, amp=0.25, width=10.,
            shape=None, xmax=600):
    ...
    state.grid.add_gauges([(390.75,)])
    state.grid.add_gauges([(590.25,)])
    state.grid.add_gauges([(590.75,)])

    state.grid.add_gauges([(190.0001,)])
    state.grid.add_gauges([(190.05,)])
    state.grid.add_gauges([(190.15,)])
    state.grid.add_gauges([(190.35,)])
    state.grid.add_gauges([(190.45,)])
    state.grid.add_gauges([(190.55,)])
    state.grid.add_gauges([(190.65,)])
    state.grid.add_gauges([(190.85,)])
    state.grid.add_gauges([(190.95,)])
    #Set global parameters
    state.problem_data = {}
    state.problem_data['t1']    = width
    state.problem_data['tw1']   = width
    state.problem_data['a1']    = amp
    ...
    state.problem_data['shape'] = shape
